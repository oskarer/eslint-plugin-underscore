...
function isMemberExpOf(node, objectName, allowComputed) {
    if (objectName) {
        if (node && node.type === 'MemberExpression' && node.object.name === objectName) {
...
function isBinaryExpWithMemberOf(operator, exp, objectName, allowComputed) {
    return exp && exp.type === 'BinaryExpression' && exp.operator === operator &&
          (isMemberExpOf(exp.left, objectName, allowComputed) ||
          isMemberExpOf(exp.right, objectName, allowComputed));
}
...
function isNegationOfMemberOf(exp, objectName) {
    return isNegationExpression(exp) && isMemberExpOf(exp.argument, objectName, false);
}
