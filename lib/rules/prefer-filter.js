...
module.exports = function (context) {
    ...
    return {
        CallExpression: function (node) {
            if (lodashUtil.isCallToMethod(node, 'each') && onlyHasSimplifiableIf(lodashUtil.getLodashIteratee(node))) {
                context.report(node, 'Prefer _.filter or _.some over an if statement inside a _.each');
            }
        }
    };
};
