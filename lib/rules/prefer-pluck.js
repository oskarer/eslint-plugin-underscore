...
module.exports = function(context) {
    ...
    return {
        CallExpression: function (node) {
            if(astUtil.getMethodName(node) === 'map') {
                if(lodashUtil.usesPropShorthand(node) || lodashUtil.couldUsePropShorthand(node)) {
                    context.report({
                        node: node.callee.property,
                        message: 'Prefer _.pluck to map when collecting a single property',
                        fix: fixerUtil.renameFunction(node, 'pluck')
                    });
                    if(lodashUtil.couldUsePropShorthand(node)) {
                        context.report({
                            node: node.callee.property,
                            message: '_.pluck expects a string, "propertyName", not a function',
